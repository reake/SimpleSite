...
!function ($) {
    ...
    $(function () {
        ...
        $("button.subscribe").on('click', function (e) {
            e.preventDefault();
            $.post('/subscribe', {email: $("input[name=email]").val()}, function (result) {
                if (result.status == 1001) {
                    $("input[name=email]").attr('readonly',true).val(result.result.msg);
                } else {
                    $("input[name=email]").val(result.result.msg);
                }
            }, 'JSON')
        })
